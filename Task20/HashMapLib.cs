...
class MyHashMap<K, V> : IEnumerable<Entry<K, V>>
{
    ...
    public void Clear()
    {
        table = new LinkedList<Entry<K, V>>[1];
    }
    ...
    public void Put(K key, V value)
    {
        ...
        {
            LinkedList<Entry<K, V>>[] newTable = new LinkedList<Entry<K, V>>[table.Length * 2];
    ...
    public void Remove(K key)
    {
        if (ContainsKey(key))
        {
            LinkedList<Entry<K, V>>[] newTable = new  LinkedList<Entry<K, V>>[table.Length];
            foreach (var list in table)
            {
                if (list != null)
                {
